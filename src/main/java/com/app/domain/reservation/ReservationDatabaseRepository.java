...
public class ReservationDatabaseRepository implements ReservationRepository {
    ...
    public Reservation createNewReservation(Room room, Guest guest, LocalDateTime from, LocalDateTime to) {
        try {
            ...
            String createTemplate = "INSERT INTO VALUES(ROOM_ID, GUEST_ID, RES_FROM, RES_TO) VALUES (%d, %d, '%s', '%s'";
            String createQuery = String.format(createTemplate, room.id(), guest.id(), fromAsStr, toAsStr);
    ...
    private void removeById(long id) {
        ...
        for (int i = 0; i < this.reservations.size(); i++) {
            if (this.reservations.get(i).id() == id) {
                indexToBeRemoved = i;
                break;
            }
        }
